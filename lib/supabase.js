...
export async function getTradesForPool(strategyType, sensitivity, limit = 20) {
  // ВАЖЛИВО: показуємо ЛИШЕ угоди, реально синхронізовані з
  // on-chain балансом клієнта (synced_to_onchain=true) - історичні
  // угоди бота ДО впровадження bot_set_user_balance інтеграції НЕ
  // впливали на реальний баланс, тож не повинні тут показуватись.
  const { data, error } = await supabase
    .from('sol_trades')
    .select('*')
    .eq('strategy_type', strategyType)
    .eq('sensitivity', sensitivity)
    .eq('synced_to_onchain', true)
    .order('created_at', { ascending: false })
    .limit(limit);
